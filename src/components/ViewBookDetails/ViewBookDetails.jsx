...
const ViewBookDetails = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get(
        `https://bookheaven-jruh.onrender.com/api/v1/get-book-by-id/${id}`
      );
  ...
  const handleFavourite = async () => {
    const response = await axios.put(
      "https://bookheaven-jruh.onrender.com/api/v1/add-book-to-favourites",
      {},
      { headers }
    );
  ...
  const handleCart = async () => {
    const response = await axios.put(
      "https://bookheaven-jruh.onrender.com/api/v1/add-to-cart",
      {},
      { headers }
    );
    alert(response.data.message);
  ...
  const deleteBook = async () => {
    const response = await axios.delete(
      "https://bookheaven-jruh.onrender.com/api/v1/delete-book",
      { headers }
    );
    alert(response.data.message);
    navigate("/all-books");
      ...
      {Data && (
        <div className="px-4 md:px-12 py-8 bg-zinc-900 flex flex-col lg:flex-row gap-8">
          ...
          <div className="p-4 w-full lg:w-3/6">
            ...
            <p className="mt-4 font-semibold text-zinc-100 text-3xl">
              Price : ₹ {Data.price}
            </p>
          </div>
        </div>
